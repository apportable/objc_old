...
static int PrintExceptions = 0;
static int PrintExceptionThrow = 0;
// extern void _objc_inform(const char *fmt, ...);
#define _objc_inform DEBUG_LOG
...
extern "C" void objc_exception_throw(id exception) __attribute__((visibility("default"))) {
    if (!xtab.throw_exc) {
...
static int default_match(Class exceptionClass, id exception) {
    ...
    Class cls;
    for (cls = object_getClass(exception); nil != cls; cls = _class_getSuperclass(cls)) 
...
typedef int (*objc_exception_matcher)(Class catch_cls, id exception);

static int _objc_default_exception_matcher(Class catch_cls, id exception)
{
    Class cls;
    for (cls = object_getClass(exception);
         cls != NULL; 
...
#ifdef APPORTABLE
extern "C" _Unwind_Reason_Code __gnustep_objcxx_personality_v0(int version,
                                                    _Unwind_Action actions,
                                                    uint64_t exceptionClass,
                                                    struct _Unwind_Exception *exceptionObject,
                                                    struct _Unwind_Context *context)
...
extern "C" void objc_exception_throw(id obj) __attribute__((visibility("default")))
{
    ...
    exc->tinfo.cls_unremapped = obj ? object_getClass(obj) : Nil;
                                     ...
                                     unsigned outer UNAVAILABLE_ATTRIBUTE)
{
    ...
    *throw_obj_p = **(void***)throw_obj_p;
    ...
    Class handler_cls = catch_tinfo->cls_unremapped;
    if (!handler_cls) {
