...
#elif ( defined(_AIX) || defined(ANDROID))
...
static void
mprotectFailed(int err, const char *source, void *address, size_t size)
{
	EF_Exit("mprotect() failed due to %d from %s on %p (%d bytes): %s", err, source, address, size, stringErrorReport());
}

void
Page_AllowAccess(void * address, size_t size)
{
	int err = mprotect((caddr_t)address, size, PROT_READ|PROT_WRITE);
	if (err < 0)
		mprotectFailed(err, "Page_AllowAccess", address, size);
}

void
Page_DenyAccess(void * address, size_t size)
{
	int err = mprotect((caddr_t)address, size, PROT_NONE);
	if (err < 0 )
		mprotectFailed(err, "Page_DenyAccess", address, size);
}
