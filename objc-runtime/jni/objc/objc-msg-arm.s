...
.macro CacheLookup /* selReg, classReg, missLabel */
	tst    $1, SMALLOBJ_MASK        // Sets Z if this is not a small int

	ldrne  $1, LSmallIntClass              // Small Int class -> r4 if this is a small int
	ldrne  $1, [$1]

	ldr    $1, [$1, #DTABLE_OFFSET]        // Dtable -> r4

	ldr    a4, LUninstalledDtable          // &uninstalled_dtable -> r5
	ldr    a4, [a4]

	teq    r4, a4                          // If dtable == &uninstalled_dtable
	beq    LSlowPath                       // Do a slow lookup

	ldr    a4, [$1, #SHIFT_OFFSET]        // dtable->shift -> r6
	ldr    r9, [$1, #DATA_OFFSET]         // dtable->data -> r4
...
2:                                        // dtable8
	...
	beq    $2 		                      // Go to the slow path and do the forwarding stuff
 ...
 *
 * _objc_msgForward_internal is passed as a parameter because its more 
